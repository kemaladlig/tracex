  ...
  const sortedPortfolio = useMemo(() => {
    ...
    if (sortBy === 'value') {
      return list.sort((a, b) =>
        (valuationById.get(b.id)?.currentValueUsd ?? 0) - (valuationById.get(a.id)?.currentValueUsd ?? 0)
      );
    }
    if (sortBy === 'pnl') {
      return list.sort((a, b) => (valuationById.get(b.id)?.pnlUsd ?? 0) - (valuationById.get(a.id)?.pnlUsd ?? 0));
    }
    return list;
  }, [portfolio, sortBy, valuationById]);
